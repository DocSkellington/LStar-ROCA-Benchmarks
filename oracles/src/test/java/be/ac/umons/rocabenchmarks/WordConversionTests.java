...
public class WordConversionTests {
    ...
    public void fromStringToSymbols() {
        ...
        Word<JSONSymbol> target = Word.fromSymbols(toSymbol("h"), toSymbol('e'), toSymbol('l'), toSymbol('l'), toSymbol('o'));
        Assert.assertEquals(result, target);

        base = "\"pro\":{\"i\":1}";
        result = WordConversion.fromStringToJSONSymbolWord(base);
        System.out.println(result);
        target = Word.fromSymbols(toSymbol('"'), toSymbol('p'), toSymbol('r'), toSymbol('o'), toSymbol("\":{"), toSymbol('"'), toSymbol('i'), toSymbol("\":"), toSymbol('1'), toSymbol('}'));
        Assert.assertEquals(result, target);
